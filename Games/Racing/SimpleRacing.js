...
(function () {
  ...
  // safety: if required elements missing, warn and abort early
  ...
  const car = {
    init() {
      this.speed = 0.2;
      this.x = carEl ? carEl.offsetLeft : 120;
      this.y = 0;
      this.width = carEl ? carEl.offsetWidth : 60;
      this.height = carEl ? carEl.offsetHeight : 28;
      this.maxSpeed = 5.0;
      this.km = 0;
      this.motor = 1;
      this.crashed = false;
    ...
    frame() {
      ...
      for (let j = 0; j < this.n; j++) {
        for (let i = 0; i < 3; i++) {
          ...
          if (!c) continue;
          const d = roadEl.width * 0.42 || 100;
          const w = (roadEl.width || 300) - d - car.width;
          c.x = (roadEl.P0.x - roadEl.height - 40) * (c.y * c.y * 0.00001) + (d / 2) + (i * (w / 2));
          c.y += relative;
          ...
          const o = 1 / (c.y * (fogEl.value || 0.02));
